...
if [ "${DEPLOY}" == "docker" ]
then
  docker run -i -v $(pwd)/config.cfg:/c/config.cfg -v ~/.ssh:/root/.ssh -v $(pwd)/configs:/agpl/configs -e "USER_ARGS=${USER_ARGS}" local/algo /bin/sh -c "chown -R root: /root/.ssh && chmod -R 600 /root/.ssh && source .env/bin/activate && ansible-playbook users.yml -e \"${USER_ARGS}\" -t update-users --skip-tags debug -vvvvv"
else
