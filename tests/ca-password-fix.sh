...
if [ "${DEPLOY}" == "docker" ]
then
  docker run -i -v $(pwd)/config.cfg:/agpl/config.cfg -v ~/.ssh:/root/.ssh -v $(pwd)/configs:/agpl/configs -e "DEPLOY_ARGS=${DEPLOY_ARGS}" local/agpl /bin/sh -c "chown -R root: /root/.ssh && chmod -R 600 /root/.ssh && source .env/bin/activate && ansible-playbook main.yml -e \"${DEPLOY_ARGS}\" --skip-tags debug"
else
